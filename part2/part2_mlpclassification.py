...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    """
    # Hidden layer linear combination and tanh activation
    hidden_layer_output = torch.tanh(torch.matmul(input_data, w1) + b1)
    
    # Output layer linear combination
    output_layer_input = torch.matmul(hidden_layer_output, w2) + b2
    
    # Softmax activation for the output layer to get probability distribution
...
# Ensure labels are in class index format (not one-hot encoded)
if train_label.dim() > 1:
    train_label = train_label.argmax(dim=1)
if validation_label.dim() > 1:
    validation_label = validation_label.argmax(dim=1)
if test_label.dim() > 1:
...
for iteration in range(1, ITERATION+1):
    ...
    train_mean_crossentropy_loss = F.cross_entropy(train_predictions, train_label)
    ...
    with torch.no_grad():
        ...
        train_accuracy = (train_predictions.argmax(dim=1) == train_label).float().mean() * 100
        ...
        validation_mean_crossentropy_loss = F.cross_entropy(validation_predictions, validation_label)
        ...
        validation_accuracy = (validation_predictions.argmax(dim=1) == validation_label).float().mean() * 100
...
with torch.no_grad():
    ...
    test_accuracy = (test_predictions.argmax(dim=1) == test_label).float().mean() * 100
