...
class MLPClassifier:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        hidden_input = np.dot(x, self.W) + self.W_bias
        hidden_layer_output = sigmoid(hidden_input)
        output_input = np.dot(hidden_layer_output, self.GAMMA) + self.GAMMA_bias
        exp_output = np.exp(output_input)
        output_layer_output = exp_output / np.sum(exp_output, axis=1).reshape(-1, 1)
    ...
    def train(self, data_instances: np.ndarray, labels: np.ndarray):
        ...
        for iteration_count in range(1, self.epoch_number+1):
            ...
            for data_index in range(len(data_instances)):
                ...
                # Compute the gradient of the loss with respect to the output layer input
                output_delta = output_error

                # Compute the gradient of the loss with respect to the hidden layer output
                hidden_error = np.dot(output_delta, self.GAMMA.T)

                # Compute the gradient of the loss with respect to the hidden layer input
                hidden_delta = hidden_error * sigmoid_derivative(hidden_layer_output)

                # Compute the weight updates
                GAMMA_update = np.dot(hidden_layer_output.T, output_delta)
                GAMMA_bias_update = np.sum(output_delta, axis=0, keepdims=True)
                W_update = np.dot(x.T, hidden_delta)
                W_bias_update = np.sum(hidden_delta, axis=0, keepdims=True)
